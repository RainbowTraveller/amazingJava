	...
	public static int maxProduct(List<Integer> numbers) {
		...
		for( Integer current : numbers) {
			...
			if( current > 0) {
				...
				maxEndingHere = Math.max(current, maxEndingHere * current); //for visual consistancy with other function
				minEndingHere = Math.min(minEndingHere * current, 1 );//Choose min appropriately or 1 is always there
			}
			if( current == 0) {
