...
public class CriticalConnections {
    ...
    private void tarjanDFSTraversal(boolean[] visited, int currNode, int parentNode) {
        ...
        for(int neighbor : neighbors[currNode]) {
            ...
            if(!visited[neighbor]) {
                ...
                lowestVisitedTimeNeighbors[currNode] = Math.min(lowestVisitedTimeNeighbors[currNode], lowestVisitedTimeNeighbors[neighbor]);
                if(visitedTimes[currNode] < lowestVisitedTimeNeighbors[neighbor]) {
                    critical.add(Arrays.asList(currNode, neighbor));
                }
            } else {
                lowestVisitedTimeNeighbors[currNode] = Math.min(lowestVisitedTimeNeighbors[currNode], visitedTimes[neighbor]);
            }
    ...
    /**
     * Pretty simple, just go through the connections given
     * and in the array for given index i, add to its adjacency list
     * the neighbor and vice versa for neighbor
     */
