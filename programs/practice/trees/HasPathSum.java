...
public class HasPathSum {
  ...
  /*
   * The idea is to subtract the node value from sum as we go down the tree
   * and check if we reach a leaf node with sum zero
   * <p> Time Complexity: O(N) where N is the number of nodes in the tree
   * <p> Space Complexity: O(H) where H is the height of the tree
   * @param root the root of the binary tree
   * @param sum the target sum
   * @return true if there is a root-to-leaf path with the given sum, false otherwise
   * Note :Simple problem but gets tricky if the values in the tree are negative.
