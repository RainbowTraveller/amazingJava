...
public class TenderRejectedCards {
    ...
    public static boolean findCardHelper (int index, int noOfCards, int balance, List<Map.Entry<Integer,
            Integer>> foundCards, List<Map.Entry<Integer, Integer>> cardEntries, int TotalNoOfCards) {
        ...
        if (balance == 0 && noOfCards == 0) {
            ...
            for (int i = index; i < cardEntries.size(); ++i) {
                // Get card at currnt index and add to list and call the function with next index and updated balance
                ...
                result = findCardHelper(index + 1, noOfCards - 1, balance - currCard.getValue(), foundCards,
                        cardEntries, TotalNoOfCards);
                // If not found
