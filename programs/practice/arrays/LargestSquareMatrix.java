...
public class LargestSquareMatrix {
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("Please enter the matrix elements in row - column format delimited by space");
...
class MatrixSizer {
    ...
    /**
     * level is important parameter, when 1 is encountered we send level 1 meaning
     * that we have obtained 1 * 1 matrix At each subsequent level or call we pass
     * level incremented by 1 if we find that the element at this position is 0, we
     * return a level that is 1 less than level passed to us. This indicates that if
     * at all the n * n matrix is at previous level
     *
     */
    private static int getMetrixSize(int row, int column, List<List<Integer>> arr, int level) {
        int currLevel = 0;
        if (row < 0 || row >= arr.size() || column < 0 || column >= arr.get(0).size()
                    ...
                    for (int j = 1; j <= columns; ++j) {
                        ...
                        if (currNum == 1) {
                            if (i - 1 >= 0 && j - 1 >= 0) {
                                ...
                                tracker[i][j] = Math.min(tracker[i - 1][j - 1],
                                        Math.min(tracker[i][j - 1], tracker[i - 1][j])) + 1;
