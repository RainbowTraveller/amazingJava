...
public class KthLargestNumber {
  ...
  /**
   * Another approach to partitioning e.g. 2 4 5 6 1 3 ==> 3 1 2 6 5 4 after first partition with
   * pivot index = 3 i.e. partitioning around pivot 3 This approach is bit tricky to understand but
   * it works fine Hoare partition scheme :
   * https://en.wikipedia.org/wiki/Quicksort#Hoare_partition_scheme
   */
  public static int partition(int[] arr, int low, int high, int pivot) {
    while (low < high) {
      while (arr[low] <= pivot) { // >= here and below for avoiding infinite loop
        low++;
      }
  ...
  public static int KthLargestPriority(int[] input, int k) {
    ...
    // By default PQ in Java is min-heap:
    // https://docs.oracle.com/javase/7/docs/api/java/util/PriorityQueue.html
    // The root node is the least element with respect to the specified ordering
    // so in the end there will be K largest elements in the PQ
