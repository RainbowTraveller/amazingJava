 ...
 *
 *  Simple way it to merge 2 arrays and then find middle element if the merged array has odd length
 *  or return average of 2 middle elements if the final array has even number of elements
 *  But this will take O(n) time.
 *
 *  To improve we
 *
 *
...
public class MedianSortedArrays {
    ...
    //When we look at the combined array as a whole, we divide it at the middle. Here partitionX and partitionY indicate
    //no. of elements on the left side of the partition. So this partition is actually in between 2 numbers( WHAT...yeah for simplicity)
    // e.g. 2 4 6 9 13
    //      1 10 11
    //
    //      After partitioning
    ...
    //      leftMaxX | rightMinX
    //      1 | 10, 11
    //    Notice here that total no. of elements is 8 which is even and each partition has 4 elements, so we managed to get a point
    //    such that all elements in left array combined are smaller than respective smallest elements in both the arrays on the right hand side
