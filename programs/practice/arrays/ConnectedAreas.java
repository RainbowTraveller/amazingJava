...
public class ConnectedAreas {
  ...
  /**
   * Constructor to initialize the topology matrix
   *
  ...
  /** Function to get the number of islands and the biggest island size */
  public void getIsland() {
    for (int i = 0; i < topology.length; ++i) {
      for (int j = 0; j < topology[0].length; ++j) {
        if (topology[i][j] == 1) {
          ...
          // System.out.println( " Current : "  + currentIogslandSize  + " Biggest : "  +
          // biggestIslandSize);
    ...
    System.out.println(
        "No of Islands :" + noOfIslands + " Biggest Island Available : " + biggestIslandSize);
  }

  /** Function to get the biggest island size using DFS */
  private int getBiggestIsland(int i, int j) {
    ...
    this.topology[i][j] = 0;
    return (1
        + getBiggestIsland(i + 1, j)
        + getBiggestIsland(i - 1, j)
        + getBiggestIsland(i, j + 1)
        + getBiggestIsland(i, j - 1));
  }
}
