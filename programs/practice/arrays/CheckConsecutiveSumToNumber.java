/*
Given a array of non negative numbers, find if there exist a consecutive subarray of at least 2 numbers
whose sum is equal to a given number
There can be zeros in the array
*/
public class CheckConsecutiveSumToNumber {
  /**
   * This is a linear time solution to the problem. It uses two pointers to maintain the start and
   * current index. The sum is maintained as we move the current index. If the sum exceeds the
   * target, we reduce the sum by moving the start index. If the sum matches the target, we check if
   * there are at least 2 numbers in the subarray.
   *
   * @param numbers array of non negative numbers
   * @param k target sum
   * @return true if there exists a consecutive subarray of at least 2 numbers whose sum is equal to
   *     k
   */
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        CheckConsecutiveSumToNumber.checkConsecutiveSumToNumberNaive(numbers, 6)); // O(n^2)
